...
import { createPhysicalOrder } from '../../api';
import { showphysicalorders } from '../../api';

export default function Store_order() {
  const [title, setTitle] = useState('實體訂單');
  ...
  const handleSaveClick = async (index) => { // 新增的處理函數
      ...
      try {
          await createPhysicalOrder(newTask); 
          alert('已成功增加訂單');
          const newTasks = [...tasks];
          newTasks[index].isEditing = false;
          setTasks(newTasks);
      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            const data = await showphysicalorders();
